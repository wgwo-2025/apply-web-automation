...
## E-sign, and why an application must reach the funding chain first

The funnel now runs to the FUNDED page. Two things had to be true first, and
neither is visible from the browser.

**1. The application must actually be advancing through sub-statuses.** A walked
application parks at Allocated (123) because underwriting-srv's Innovis check
adds `Manual Escalation (107)` and `Fraud Pattern Tracking (210)` during the
fraud stage, and rule 217 (`Application - AutoPay Data Submitted`) requires
portfolios 103/107/108/113/114/115 to be inactive. Remove 107 and 210 and the
whole chain runs on its own in about 90 seconds:

```
Status -> TIL Generation Requested
fund    ePackage envelope id updated        <- the DocuSign envelope
fund    Doc: Truth in Lending (TIL)         <- the TIL PDF, generated by automation
Status -> TIL Generation Complete (104)
```

Until that chain runs there is no envelope, and `POST /fund/applications/:id/embedded`
answers **500 "Envelope wasnt generated for this application"**. Generating a TIL
document by hand gets you past the TIL page but NOT past e-sign.

**2. `Email (cf278)` must be set.** apply-bff maps `loanSettings.borrowerEmail`
from it, and `generateDocusignUrl` throws client-side without it — the POST never
even leaves the browser, and apply-web's fallback `navigate()` builds a relative
path (`/fund/truth-in-lending/:id/docusign-callback/:id`) that matches no route,
so the borrower lands on `/error`. That fallback is an apply-web bug: a failed
envelope shows "Something went wrong!" rather than the callback page.

### Driving the DocuSign page

It is a third party, so `signDocuSign` matches on `data-qa` rather than text.
Measured on the demo environment 2026-09-12: an "AI-Assisted" callout opens over
the document and eats the first click, and `data-qa` still reads
`-tab-required-` after a field is signed — the innerText is what flips to
"Signature Applied", so that is the completion check. The adopt dialog appears
only for the first signature of a session.
